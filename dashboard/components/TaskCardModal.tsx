...
const TYPE_CONFIG: Record<ItemType, { icon: React.ReactNode; label: string; color: string }> = {
    budget:   { icon: <DollarSign className="w-3 h-3" />, label: 'Budget',   color: 'var(--nerv-warn)' },
    stack:    { icon: <Code2 className="w-3 h-3" />,      label: 'Stack',    color: 'var(--nerv-cyan)' },
    legal:    { icon: <Scale className="w-3 h-3" />,      label: 'Legal',    color: 'var(--nerv-violet)' },
    deadline: { icon: <Clock className="w-3 h-3" />,      label: 'Deadline', color: 'var(--nerv-danger)' },
    custom:   { icon: <Shield className="w-3 h-3" />,     label: 'Custom',   color: 'var(--nerv-text-secondary)' },
};
    ...
    return (
        <div ref={ref} className="absolute bottom-full left-0 mb-2 z-50 rounded-md overflow-hidden shadow-lg"
            ...
            <div className="p-3 space-y-2">
                <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: 'var(--nerv-text-tertiary)' }}>
                    {title}
                </span>
                ...
                <button
                    ...
                    style={{ border: '1px dashed var(--border)', color: 'var(--nerv-text-ghost)' }}>
                ...
                <div className="space-y-1.5">
                    <div className="flex items-center gap-1.5">
                        <span className="shrink-0 opacity-60" style={{ color: 'var(--nerv-text-secondary)' }}>
                            <Plus className="w-3 h-3" />
                        </span>
            ...
            <DialogContent showCloseButton={false}
                ...
                {isAutoGenerating && (
                    <div className="px-5 py-2.5" style={{ borderBottom: '1px solid color-mix(in srgb, var(--border) 30%, transparent)' }}>
                        <div className="flex items-center gap-2 text-[11px] text-muted-foreground animate-pulse">
                            <Loader2 className="w-3.5 h-3.5 animate-spin" style={{ color: 'var(--nerv-warn)' }} />
                ...
                <div className={cn("flex-1 overflow-y-auto px-5 py-5 space-y-4 min-h-0", isAutoGenerating && "opacity-40 pointer-events-none")}>
                                    ...
                                    <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                                        ...
                                        {agents.map(a => (
                                            <button key={a.id} onClick={() => { setAgentId(a.id); setShowAgentDropdown(false); }}
                                                ...
                                                {a.id === agentId && <Check className="w-3 h-3 ml-auto" style={{ color: 'var(--nerv-success)' }} />}
                                            </button>
                                        ))}
                                    </motion.div>
                                )}
                    ...
                    <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                            ...
                            <button onClick={addStep} className="flex items-center gap-1 text-[10px] transition-colors px-1.5 py-0.5 rounded-sm hover:bg-white/5" style={{ color: 'var(--nerv-cyan)' }}>
                        ...
                        <div className="space-y-1">
                            ...
                            {steps.length === 0 && (
                                <button onClick={addStep} className="w-full flex items-center justify-center gap-1.5 py-3 rounded-md text-[11px] transition-all hover:bg-white/3"
                                    style={{ border: '1px dashed var(--border)', color: 'var(--nerv-text-ghost)' }}><Plus className="w-3 h-3" /> Add execution steps</button>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap pt-1">
                        ...
                        <div className="relative">
                            <button onClick={() => setShowGoalPopover(!showGoalPopover)}
                                ...
                                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'var(--nerv-cyan)'; }}
                                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = 'var(--nerv-text-ghost)'; }}>
                        ...
                        <div className="relative">
                            <button onClick={() => setShowConstraintPopover(!showConstraintPopover)}
                                className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-sm text-[10px] transition-all hover:bg-white/5"
                                style={{ color: 'var(--nerv-text-ghost)' }}
                                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'var(--nerv-warn)'; }}
                                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = 'var(--nerv-text-ghost)'; }}>
                ...
                <div className="flex items-center justify-end gap-2 px-5 py-4 shrink-0"
                    ...
                    <Button size="sm" onClick={handleHandoff}
                        ...
                        style={{ background: isSubmitting ? 'var(--nerv-success)' : 'rgb(234, 120, 47)', color: '#fff' }}>
