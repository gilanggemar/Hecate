...
export function PopoverForm({
  ...
  return (
    ...
    >
      <div className="opacity-0 pointer-events-none flex items-center justify-center" aria-hidden="true">
        {triggerNode ? (
          <div className="outline-none">{triggerNode}</div>
        ) : (
          <div className="flex h-9 items-center border border-transparent px-3 text-sm font-medium">
            <span>{title}</span>
          </div>
        )}
      </div>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <AnimatePresence initial={false}>
          {!open ? (
            <motion.button
              layoutId={`${uniqueId}-wrapper`}
              key="button"
              onClick={() => setOpen(true)}
              style={{ borderRadius: 8 }}
              className={triggerNode ? "outline-none" : "flex h-9 items-center border bg-white dark:bg-[#121212] px-3 text-sm font-medium outline-none"}
            >
              <motion.span layoutId={`${uniqueId}-title`}>
                {triggerNode ? triggerNode : title}
              </motion.span>
            </motion.button>
          ) : (
            <motion.div
              ...
              style={{
                ...
                top: '50%',
                left: '50%',
                marginLeft: width && width.includes('px') ? `-${parseInt(width) / 2}px` : '-182px',
                marginTop: height === 'auto' ? '-110px' : (height && height.includes('px') ? `-${parseInt(height) / 2}px` : '-96px')
              }}
            >
              <motion.span
                aria-hidden
                className="absolute left-4 top-[17px] text-sm text-muted-foreground data-[success]:text-transparent"
                layoutId={`${uniqueId}-title`}
