...
// e.g. change TOOL_PREFIX to rebrand all 70+ tools (currently "hecate")
// ──────────────────────────────────────────────────────────────────────────────

export const TOOL_PREFIX = "hecate";
...
export function getUserId(): string {
    const id = process.env.HECATE_USER_ID;
    if (!id) throw new Error("HECATE_USER_ID environment variable is required");
    return id;
}

/** Server metadata */
export const SERVER_NAME = "hecate-dashboard";
export const SERVER_VERSION = "1.0.0";
export const SERVER_DESCRIPTION =
    "MCP server for full programmatic control of the Hecate dashboard. " +
